...
runLog.log("C")
      ...
      // handling. No need to throw the error farther.
      if (!stopped)
        throw e;
    }
  };
  ...
  try {
    if (options.multiple) {
      ...
      _.each(_.range(3), function (i) {
        ...
        runLog.log("starting a mongo" + stopped + i)
        if (stopped)
          return;
...
var MongoRunner = function (options) {
  ...
  self.multiple = true || !!options.multiple;  // XXX delete true||
...
_.extend(MongoRunner.prototype, {
  ...
  start: function () {
    ...
    // Keep trying to start the process until either (a) we successfully start
    // up or (b) something tries to .stop() us (eg, ourselves in _exited).
    while (!self.handle && !self.shuttingDown) {
